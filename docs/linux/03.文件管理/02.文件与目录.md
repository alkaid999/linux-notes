---
title: 文件与目录
sidebarPrefix: <i class='iconfont icon-mulu'></i>
---
...
- **长度限制**： 文件和目录名的长度通常限制为 255 个字符，这是由许多文件系统（如 ext4、XFS）的实现决定
...
- **避免使用空格**：文件名或目录名中避免使用空格，因为空格会被解释为分隔符。如果需要在文件名或目录名中包含空格，请使用下划线 `_` 或短横线 `-` 等替代字符
...
- **当前目录**：符号 `.` 代表当前工作目录，即当前所在的目录。当执行命令或指定文件路径时，如果路径以 `.` 开头，它表示这个文件或目录位于当前工作目录中
- **父目录**：符号 `..` 代表当前工作目录的父目录。它用于在路径中向上移动一级，即从当前目录回到其父目录
