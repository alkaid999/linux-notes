---
title: 文件操作命令
sidebarPrefix: <i class='iconfont icon-wenjiancaozuo'></i>
---
...
- `-L`：逻辑模式（默认行为）。跟随软链接，将软链接视为目录
- `-P`：物理模式。解析路径中的所有软链接，使用真实的物理路径
- `-e`：如果使用了 `-P` 选项，并且当前工作目录无法确定，那么 `cd` 命令将退出并返回一个非零状态。这通常用于脚本中，以便在目录切换失败时能够检测到错误
- `-@`：在支持的系统上，这个选项会将具有扩展属性的文件显示为一个目录，其中包含文件的扩展属性
:::

::: field name="DIR" type="目录" optional default="用户家目录"
表示要切换到的目录。路径可以是绝对路径，也可以是相对路径，若省略不写则会跳转至当前使用者的家目录。
:::
...
:::: field-group
::: field name="OPTIONS" type="常用选项" optional
...
- `-h`：与 `-l` 一起使用时，以易读的格式（如 K、M、G 表示千字节、兆字节和吉字节）显示文件大小
- `-i`：显示文件的 inode 号
- `-d`：仅列出目录本身
- `-R`：递归地列出所有子目录的内容
- `-f`：直接列出文件和目录，不进行任何排序
...
::: field name="FILE" type="文件或目录" optional default="当前工作目录"
想要列出其内容的文件或目录的路径，如果没有指定，则默认为当前工作目录。
:::
...
- 查看目录下所有文件
    ...
    除此之外，还使用了 `-l` 选项，因此才显示出了文件的详细信息，具体包括以下七个方面的信息：
...
::: field name="DIRECTORY" type="目录" required
目录名，想要创建的一个或多个目录的名称，可以是单个目录名，也可以是包含多个子目录的路径
:::
...
::: field name="FILE" type="文件" required
文件名，可以是单个文件名，也可以是包含多个文件的路径。如果文件已存在，将根据选项更新其时间戳。
:::
...
> [!TIP] STRING 可以是以下形式
>
> - 标准日期格式：例如： `YYYY-MM-DD HH:MM:SS` 的格式，2024-09-19 17:01:05
> - 相对日期格式：例如 `10 minutes ago`、`2 hours ago`、`1 day ago` 等，用于指定相对于当前时间的过去时间
> - 使用文字描述：例如 `now`、`yesterday`、`next Friday`、`last Monday` 等，用于描述特定的日期和时间
...
- 创建一个文件，然后使用 `touch` 仅更新文件的访问时间
    ...
    可以看到文件 yy 的 Access 和 Change 时间更新了，因为更改文件的任何元数据（包括访问时间）时，所以会同时更新状态改变时间。
...
- 删除空目录
    ...
    可以看到空目录 `testdir` 已经被删除了，但是 `rmdir` 命令的作用十分有限，因为只能刪除空目录。

    假设 `testdir` 目录中存在 1.txt 这个文件：
    ...
    ```console
    total 0
    -rw-r--r-- 1 root root 0 Jan  4 11:28 1.txt
    ```
...
::: field name="FILE" type="文件" required
可以指定一个或多个文件或目录作为命令的目标，这些是将要被删除的文件或目录。
:::
...
::: field name="OPTION" type="常用选项" optional default="创建硬链接"

- `-s`：创建软链接（符号链接）而不是硬链接
- `-v`：详细模式，在创建链接时显示额外的信息
- `-f`：强制模式，如果目标文件已存在，先删除它
...
::: field name="TARGET" type="文件或目录" required
链接的源文件或目录。它可以是一个文件或多个文件或目录的路径，也就是要链接的对象。
:::

::: field name="DIRECTORY" type="文件或目录" required
链接放置的目标目录。它是链接文件或目录的存放位置。也就是创建链接的名称。
:::
...
软链接也称为符号链接，是将一个路径名链接到一个文件。实际上，软链接只是一个文本文件，其中包含它提供链接的另一个文件的路径名（另一个文件是实际包含所有数据的文件）。所有读、写文件内容的命令被用于软链接时，将沿着链接方向前进来访问实际的文件。

*类似于 Windows 的快捷方式，可以快速链接到目标文件或目录。*

软链接与硬链接不同，它是一个全新的文件，具有自己的 inode 号码。软链接包含了目标文件或目录的路径，而不是直接指向它们的 inode。这给了软链接一些独特的特性：
...
- `-u`：只复制修改过的文件，即源文件比目标文件新

:::

::: field name="SOURCE" type="源文件/目录" required
一个或多个要复制的源文件或目录的名称。默认情况下，`cp` 命令不能复制目录，如果要复制目录，则必须使用 `-r` 选项。
:::

::: field name="DIRECTORY" type="目标文件/目录" required
指定目标文件/目录。当源文件为多个文件时，要求目标文件为指定的目录，且目录必须已存在。
:::
...
::: field name="SOURCE" type="源文件/目录" required
表示一个或多个源文件或目录，这些是要移动或重命名的文件或目录。
:::

::: field name="DIRECTORY" type="目标文件/目录" required
目标目录，这是要将源文件或目录移动到的位置。
:::
...
::: field name="FILE" type="目录" required
表示是要显示树形结构的目录，如果不指定目录路径，`tree` 命令默认会从当前目录开始显示。
:::
...
::: field name="OPTION" type="选项" optional
控制 `find` 命令的全局行为​（如符号链接处理），必须放在最开头。
:::

::: field name="PATH" type="路径" optional default="当前目录"
指定搜索的起始目录，可多个路径。默认值为当前目录。
:::

::: field name="EXPRESSION" type="表达式" optional
文档核心，由测试条件 + 操作符 + 动作组成，用于筛选文件并执行操作
:::
...
::: field name="OPTION" type="表达式选项" optional
...
在 `find` 命令中，测试条件（Tests） 是表达式（Expression）的核心组成部分，用于判断文件是否满足特定属性（如名称、类型、大小、时间等）。

- 每个测试返回真（`true`）或假（`false`）
- 多个测试可组合使用（默认用 `-a`/`and` 连接）
- 若未指定动作（如 `-print`、`-delete`），且测试为真，`find` 通常会默认打印路径

---

文件名匹配，用于根据文件名或路径进行匹配：

::: field name="Tests" type="文件名匹配" optional

- `-name "PATTERN"`：匹配 **文件名**（不含目录部分），区分大小写，支持通配符
- `-iname "PATTERN`：同 `-name`，但 **不区分大小写**
- `-path "PATTERN"`：匹配 **完整路径**（从起始目录开始的相对或绝对路径），区分大小写，使用相同通配符规则
- `-ipath "PATTERN"`：同 `-path`，但 **不区分大小写**
- `-regex "PATTERN"`：使用正则表达式 **完全匹配完整路径**（隐含 `^...$`）
- `-iregex "PATTERN"`：同 `-regex`，但 **不区分大小写**
:::

> [!TIP]
> 通配符中 `*` 匹配任意字符（包括 `/` 在 `-path` 中），而正则中需用 `.*`。
>
> 使用 `-regextype posix-extended` 可启用扩展正则（如 `+`, `|`）。
...
文件类型，用于筛选不同类型的文件系统对象：

::: field name="Tests" type="文件类型" optional
...
- `atime`：文件最后被访问（读取）的时间
- `ctime`：文件状态（元数据）最后变更时间（如权限、所有者、硬链接数等；不是创建时间）

::: field name="Tests" type="时间" optional

按天（单位：24 小时）：

- `-mtime N`：内容修改时间（N 天前）
- `-atime N`：文件访问时间（N 天前）
- `-ctime N`：状态变更时间（权限/所有者等，N 天前）
...
- `-newer FILE`：查找 mtime 比 FILE 的 mtime 更新 的文件（即修改时间更晚）
- `-anewer FILE`：查找 atime 比 FILE 的 mtime 更新 的文件
- `-cnewer FILE`：查找 ctime 比 FILE 的 mtime 更新 的文件
:::
...
按文件字节大小筛选，适用于清理大文件或空文件：

::: field name="Tests" type="文件大小" optional

- `size N`：文件大小 恰好等于 N 单位
- `size +N`：文件大小 大于 N 单位
- `size -N`：文件大小 小于 N 单位
:::
...
用于根据文件权限位或所有者 / 所属组进行筛选：

::: field name="Tests" type="权限与用户" optional

权限匹配：

- `-perm MODE`：权限 **完全等于** MODE
- `-perm -MODE`：权限 **包含 MODE** 中的所有位（即至少具有这些权限）
- `-perm +MODE`：权限 **包含 MODE** 中的任意一位（即至少具有这些权限）

用户组匹配：

- `-user USERNAME` 或 `-user UID`：文件所有者是指定用户（可用用户名或数字 UID）
- `-group GROUPNAME` 或 `-group GID`：文件所属组是指定组（可用组名或数字 GID）
- `-nouser`：文件没有对应的有效用户（如 UID 不在 `/etc/passwd` 中）
- `-nogroup`：文件没有对应的有效组（如 GID 不在 `/etc/group` 中）
:::
...
提供额外实用功能，如检测空文件、当前用户权限、逻辑控制等：

::: field name="Tests" type="其他" optional

- `-empty`：匹配 **空文件**（大小为 0 字节）或 **空目录**（不含任何子项）
- `-readable`：匹配当前运行 `find` 的用户 **有读权限** 的文件或目录
- `-writable`：匹配当前用户 **有写权限** 的文件或目录
- `-executable`：匹配当前用户 **有执行权限** 的文件，或 **有进入权限** 的目录
- `-true`：始终返回 `true`，匹配所有文件（可用于显式启用默认行为）
- `-false`：始终返回 `false`，不匹配任何文件（常用于调试或屏蔽条件）
:::

> [!TIP]
> 这些选项基于当前用户的实际权限动态判断，比手动解析 `-perm` 更直观。
...
在 `find` 命令中，操作符（Operators）用于组合、控制或修改多个测试条件（Tests）和动作（Actions）的逻辑关系。通过操作符，可以构建复杂的筛选表达式，实现精确匹配。

- 默认情况下，相邻的测试条件以逻辑与（`AND`） 连接
- 操作符的优先级从高到低为：`()`（分组） > `!`（非） > `-a`（与） > `-o`（或）
- 所有操作符都作用于表达式求值过程，最终决定是否执行后续动作（如 `-print`）

::: field name="Operators" type="逻辑与组合" optional default="-a"

- `$… $`：分组表达式，用于改变操作符优先级；在 shell 中必须用反斜杠转义
- `! EXPR` 或 `-not EXPR`：对表达式 EXPR 执行逻辑非（取反）
- `EXPR1 -a EXPR2` 或 `EXPR1 -and EXPR2`：逻辑与，仅当两个表达式均为真时结果为真（默认连接方式，通常可省略）
- `EXPR1 -o EXPR2` 或 `EXPR1 -or EXPR2`：逻辑或，只要任一表达式为真，结果即为真
- `EXPR1 , EXPR2`：顺序求值操作符，先执行 EXPR1，再执行 EXPR2，最终返回 EXPR2 的求值结果，忽略 EXPR1 的真假
:::
...
在 `find` 命令中，动作（Actions）是表达式的执行部分，用于对匹配的文件执行特定操作。若表达式中未显式指定动作，`find` 默认会执行 `-print`（打印文件路径）。每个动作本身也返回真值，可参与逻辑组合。

::: field name="Actions" type="动作" optional default="-print"

- `-print`：将匹配文件的路径输出到标准输出，每条路径以换行符结尾（默认动作）
- `-print0`：将匹配文件的路径输出到标准输出，每条路径以空字符（`\0`）结尾，适用于配合 `xargs -0` 处理含空格或特殊字符的文件名
- `-printf FORMAT`：按指定格式输出文件信息（支持类似 C 语言 `printf` 的转义序列，如 `%f` 文件名、`%p` 路径、`%s` 大小等）
- `-ls`：以 `ls -dils` 格式列出匹配文件的详细信息（包括 inode、权限、链接数、用户、组、大小、时间、路径）
- `-delete`：删除匹配的文件或空目录（不能删除非空目录）；隐含 `-depth` 模式，且必须放在表达式末尾
- `-exec COMMAND {} \;`：对每个匹配文件执行指定命令，`{}` 被替换为文件路径，命令以 `\;` 结尾
- `-exec COMMAND {} +`：将多个匹配文件路径作为参数批量传给命令，减少进程调用次数，效率高
- `-ok COMMAND {} \;`：与 `-exec ... \;` 类似，但在执行前会提示用户确认（交互式）
- `-fls FILE`：将 `-ls` 的输出写入指定文件 FILE，而非标准输出
- `-fprint FILE`：将 `-print` 的输出写入指定文件 FILE，而非标准输出
- `-fprint0 FILE`：将 `-print0` 的输出写入指定文件 FILE，每条路径以空字符（`\0`）结尾
- `-quit`：一旦找到第一个匹配项并完成其动作，立即退出 `find`，不再继续搜索
:::
